...
export default function HomeHeroSection() {
  ...
  const fetchBuildingPolygon = (lat: number, lon: number, map: google.maps.Map) => {
    const query = `
      [out:json];
      (
  ...
  const getClosestBuilding = (elements: any[], lat: number, lon: number) => {
    return elements.reduce((acc, building) => {
    ...
    }, { minDist: Infinity, building: null }).building
  }

  const getCentroid = (coords: number[][]): number[] => {
    const [latSum, lonSum] = coords.reduce(([latAcc, lonAcc], [lat, lon]) => [latAcc + lat, lonAcc + lon], [0, 0])
    return [latSum / coords.length, lonSum / coords.length]
  }

  const haversineDistance = ([lat1, lon1]: number[], [lat2, lon2]: number[]): number => {
  ...
  const updateAreaDisplay = (text: string) => {
  ...
  return (
    <div className="relative">
      <div className={`relative flex w-full items-center overflow-hidden ${currentStep === 1 ? "min-h-[500px]" : "min-h-[350px]"}`}>
        ...
        <div className="container relative z-10 mx-auto flex flex-col items-center px-5 py-6 md:flex-row md:items-start md:gap-16 lg:gap-24 lg:px-10 lg:py-12">
          ...
          <div className={`w-full ${currentStep === 1
